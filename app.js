...
// ANONYMOUS FUNCTION bi bilo da smo napisali funkciju direktno u zagradu ('click', function(){})
    ...
    } else if (document.querySelector('.btn-roll').innerHTML == '<i class="ion-ios-loop"></i>NEXT PLAYER')
    {
      ...
      if (activePlayer == 1) { 
        ...
        // dark mode switch div has to change color with player change
...
if (activePlayer == 1) { 
  ...
  // Isto to samo za dark mode switch div
  document.querySelector('.custom-switch').classList.remove('custom-control');
  document.querySelector('.custom-switch').classList.add('custom-control2');
...
// Refresh broja pobjeda u counterima dolje
...
/*DARK MODE IMPLEMENTACIJA - funkcije za promjene stanja, funkcija za flip koja ih objedinjuje, event listener jquery*/
function darkMode() {
    document.getElementById('styleMain').href = 'style-black.css';
    document.querySelector('.custom-control-label').textContent = 'LightMode';
    dark = true;
}

function lightMode() {
  document.getElementById('styleMain').href = 'style.css';
  document.querySelector('.custom-control-label').textContent = 'DarkMode';
  dark = false;
}

function flip () {
  dark == false ? darkMode() : lightMode();
}



// Funkcionalnost Bootstrap switcha, jquery
$('#customSwitch1').on('change.bootstrapSwitch', flip);